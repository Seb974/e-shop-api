...
final class GoodFilterByStatus extends AbstractContextAwareFilter
{
    protected function filterProperty(string $property, $value, QueryBuilder $queryBuilder, QueryNameGeneratorInterface $queryNameGenerator, string $resourceClass, string $operationName = null)
    {
        if ($resourceClass !== Good::class && (!$this->isPropertyEnabled($property, $resourceClass) || !$this->isPropertyMapped($property, $resourceClass))) {
            return;
        } else if ($resourceClass == Good::class && $property != "active") {
            return;
        }
        ...
        $queryBuilder
            ->leftJoin("$rootAlias.provision","m")
            ->andWhere(sprintf('m.status IN (:%s)', $parameterName))
            ->setParameter($parameterName, ["ORDERED"]);
    }
